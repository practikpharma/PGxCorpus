require "nn"
dofile("../layers/ConditionedLinear.lua")
...
local clinear = nn.ConditionedLinear(inputsize, outputsize, true, nconditions)
...
for i=1, clinear.weight:size(1) do
   for j=1, clinear.weight:size(2) do
      ...
      --print(deriv - clinear.gradWeight[i][j])
      assert(math.abs(deriv - clinear.gradWeight[i][j]) < 0.01)
...
for i=1, clinear.bias:size(1) do
   ...
   print(deriv - clinear.gradBias[i])
   assert(math.abs(deriv - clinear.gradBias[i]) < 0.01)
...
for i=1, input:size(1) do
   ...
   print(deriv - gradinput[i])
   assert(math.abs(deriv - gradinput[i]) < 0.00001)
...
print(cond)
local target = torch.rand(ncomp, outputsize)
local criterion = nn.MSECriterion()
